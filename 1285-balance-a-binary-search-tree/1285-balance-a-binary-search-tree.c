...
avl_node InsertCaller(avl_node avl_root,struct TreeNode* root){
    if(root == NULL)
        return avl_root;
    avl_root = Insert(avl_root,root->val);
    avl_root = InsertCaller(avl_root,root->left);
    avl_root = InsertCaller(avl_root,root->right);
    return avl_root;
}

struct TreeNode* balanceBST(struct TreeNode* root) {
    avl_node avl_root = NULL;
    avl_root = InsertCaller(avl_root,root);
